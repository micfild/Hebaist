...
Tile.prototype.draw = function () {
    ...
    //créer l'element image
    oImg.setAttribute('src', "assets/pics/face_cachee.jpg");
    ...
    //enregistre cette fonction (affichage du coté face de la Tile ) au clic sur la face cachéé de chaque image
...
Tile.prototype.drawFaceDown = function () {
    ...
    img.src="assets/pics/face_cachee.jpg";
    this.visible = false;
...
Tile.prototype.drawFaceUp = function() {
    ...
    // si 2 cartes on été retourné, on ne tient plus compte des clics
    if(jeu.carte_retournee.length >= 2){
        return 0;
    }


    //garde en mémoire la carte retounée
    ...
    //lorsque 2 cartes sont retounée, on réalise les tests
    // - identique ?
    // - fin de partie ?
    // - ...
    ...
    if(jeu.carte_retournee.length == 2){
        ...
        if(cartes_identiques()){
            ...
            // permet de ne pas tenir compte des click pour les tile déja retrouné
            jeu.carte_retournee.forEach(function(element) {
                jeu.carte_retournee_total.push(element);;
            });

            //on remet a vide les carte retouné pendant ce coup
            // et incrément le nombre de paire déja trouvé
            jeu.carte_retournee = [];
            jeu.nb_paire_trouve++;
...
function reset(){
    ...
    //chque carte actuellement retounée va etre remis face cachée
    jeu.carte_retournee.forEach(function(element) {
...
function cartes_identiques(){
    ...
    if( carte_1.id != carte_2.id && carte_1.face == carte_2.face){
        ...
        // les cartes ne peuvent plus être cliquée
...
for (var i = 1; i <= NUM_CARTE; i++) {
    ...
    for (var j = 1; j <= 2; j++) {
        selected.push("assets/pics/mini" + i + ".jpg");
    }
}
...
// pour éviter les drag and drop intempestif lorsque cliqeu sur les images
